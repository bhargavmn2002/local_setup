...
async function getDisplayFromDeviceToken(req) {
  ...
  try {
    ...
    // CRITICAL SECURITY FIX: Verify the device token matches the display's organization
    // This prevents cross-tenant access where one display could access another organization's content
    if (decoded.displayId !== display.id) {
      console.warn('🚨 [SECURITY] Device token displayId mismatch:', {
        tokenDisplayId: decoded.displayId,
        actualDisplayId: display.id,
        displayName: display.name
      });
      return null;
    }

    // Additional verification: ensure the managedByUserId in token matches the display record
    if (decoded.managedByUserId && decoded.managedByUserId !== display.managedByUserId) {
      console.warn('🚨 [SECURITY] Device token managedByUserId mismatch:', {
        tokenManagedByUserId: decoded.managedByUserId,
        displayManagedByUserId: display.managedByUserId,
    ...
    console.error('🚨 [DEBUG] Database error in getDisplayFromDeviceToken:', error.message);
    return null;
  }
}
