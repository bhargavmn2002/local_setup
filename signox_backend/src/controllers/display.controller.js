...
exports.pairDisplay = async (req, res) => {
  try {
    ...
    // ============================================
    // 🔐 LICENSE LIMIT ENFORCEMENT (SaaS) - Individual USER_ADMIN Limits
    // ============================================
    if (clientAdminId) {
      // For USER_ADMIN, check their individual display limit
      if (req.user.role === 'USER_ADMIN') {
        const userAdminProfile = await prisma.userAdminProfile.findUnique({
          where: { userAdminId: req.user.id },
        });

        const maxDisplays = userAdminProfile?.maxDisplays ?? 10;

        // Count currently active displays for this USER_ADMIN only
        const activeDisplays = await prisma.display.count({
          where: {
            managedByUserId: req.user.id,
            isPaired: true,
            status: { not: 'PAIRING' },
          },
        });
        ...
        // For CLIENT_ADMIN, check aggregate limits across all their USER_ADMINs
        ...
        // Count currently active displays for this client
        const activeDisplays = await prisma.display.count({
          where: {
            clientAdminId,
            isPaired: true,
        ...
        if (activeDisplays >= totalMaxDisplays) {
          return res.status(403).json({
